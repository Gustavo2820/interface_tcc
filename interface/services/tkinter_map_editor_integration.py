...
class TkinterMapEditorIntegration:
    ...
    def launch_tkinter_editor(self, width_px: int = 20, height_px: int = 20, cell_size: int = 25) -> bool:
        ...
        try:
            ...
            # Build a safe Python script string. Use triple quotes only for the outer
            # f-string; avoid stray triple quotes inside the content.
            script_content = (
                "import sys\n"
                f"sys.path.append('{map_creation_module}')\n"
                "from pixel_map_editor import create_map_editor\n"
                "import os\n"
                "\n"
                "# Configurar diretório de salvamento\n"
                f"auto_save_dir = '{self.mapas_dir.absolute()}'\n"
                "\n"
                "# Map editor launcher generated by Streamlit\n"
                "# Criar e executar editor (cols = largura, rows = altura)\n"
                f"editor = create_map_editor(\n"
                f"    rows={height_px}, \n"
                f"    cols={width_px}, \n"
                f"    cell_size={cell_size},\n"
                f"    auto_save_dir=auto_save_dir,\n"
                f"    web_integration=True\n"
                ")\n"
                "editor.mainloop()\n"
            )
