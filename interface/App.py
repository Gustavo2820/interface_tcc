...
st.markdown("""
    ...
    .folders { display: flex; justify-content: center; gap: 60px; flex-wrap: wrap; }
    .folder {
        width: 200px; height: 140px; border: 2px solid #001f3f; border-radius: 8px;
        background-color: #083d77; color: white; font-size: 18px; font-weight: 500;
        text-align: center; line-height: 140px; cursor: pointer; position: relative;
        box-shadow: 2px 4px 6px rgba(0,0,0,0.1); transition: all 0.2s ease-in-out; text-decoration: none;
    }
    .folder::before {
        content: ""; position: absolute; top: -20px; left: 0; width: 80px; height: 20px;
        background-color: #f8f4e3; border: 2px solid #001f3f; border-bottom: none; border-radius: 6px 6px 0 0;
    }
    .folder:hover { transform: scale(1.07); box-shadow: 4px 8px 12px rgba(0,0,0,0.2); }
    </style>
...
# ================= BOTÃO RESET =================
if st.button("Resetar Interface / Limpar Cache"):
    # Limpa session_state
    for key in list(st.session_state.keys()):
        del st.session_state[key]
    # Limpa caches de funções e recursos
    try:
        st.cache_data.clear()
        st.cache_resource.clear()
    except AttributeError:
        pass  # fallback para versões antigas
    st.warning("Interface resetada! Por favor, recarregue a página (F5).")
