...
class Scenario(object):
    ...
    def __init__(self, experiment, draw=False, scenario_seed=0, simulation_seed=0, directory=''):
        ...
        self.root_path = os.path.dirname(os.path.dirname(os.path.abspath(__file__))) + self.sep
        
        self.root_path2 = self.root_path + "output" + self.sep
    ...
    def load_wall_map(self):
        ...
        if (self.draw):
            self.wall_map.draw_map(self.root_path + "input" + self.sep + self.directory)
    ...
    def load_static_map(self):
        ...
        if (self.draw):
            self.static_map.draw_map(self.root_path + "input" + self.sep + self.directory)
        ...
        with open(self.root_path + "input" + self.sep + self.directory + self.sep + "individuals.json", 'r') as json_file:
            ...
            if isinstance(data, list):
                for individual_data in data:
                    ...
                    self.individuals.append(Individual(individual_data, individual_data.get('row', 0), individual_data.get('col', 0)))
            else:
